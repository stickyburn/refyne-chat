...
import { type Unsubscriber, type Writable, get } from 'svelte/store';
...
const submitPrompt = async (userPrompt, { _raw = false } = {}) => {
	...
	// focus on chat input (desktop only - mobile keyboards cause UX issues)
	if (!$mobile) {
		const chatInput = document.getElementById('chat-input');
		chatInput?.focus();
	}
	...
	// dont autofocus on mobile, so keyboard doesnt pop up
	if ($mobile) {
		messageInput?.blur();
	}
};
...
const sendMessageSocket = async (model, _messages, _history, responseMessageId, _chatId) => {
	...
	for (const toolId of selectedToolIds) {
		if (toolId.startsWith('direct_server:')) {
			...
			if (!isNaN(Number.parseInt(serverId))) {
				toolServerIds.push(Number.parseInt(serverId));
			} else {
			...
			if (typeof message.content === 'string') {
				...
				return {
					...
					content: message.content.map((part) =>
						part.type === 'text' ? { ...part, text: part.text.replace(/<\$[^>]+>/g, '').trim() } : part
					)
				};
			}
			return message;
